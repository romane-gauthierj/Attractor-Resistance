...
def vizualise_table_phenotype_condition(
    folder, patient_resistant_mean, patient_sensitive_mean
):
    ...
    output_path = (
        f"{folder}/sensitive_resistant_results/table_expression_per_phenotype.png"
    )
    plt.savefig(output_path, dpi=300, bbox_inches="tight")
    df.to_csv(
        f"{folder}/sensitive_resistant_results/table_expression_per_phenotype.csv",
        index=False,
    )
...
def plot_side_by_side_heatmaps(resistant_mean, sensitive_mean, folder_results):
    ...
    plt.tight_layout()
    plt.savefig(
        f"{folder_results}/sensitive_resistant_results/heatmap_resistant_vs_sensitive.png",
        dpi=300,
    )
    plt.show()
