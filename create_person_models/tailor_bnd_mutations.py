...
def personalized_patients_mutations_bnds(
    mutations_data, patients_ids, bnd_dir, drug_interest
):
    ...
    kras_pattern = re.compile(
        r"Node\s+KRAS\s*\{[^{}]*?(?:\{[^{}]*?\}[^{}]*?)*\}", re.DOTALL,
    )
    egfr_pattern = re.compile(
        r"Node\s+EGFR\s*\{[^{}]*?(?:\{[^{}]*?\}[^{}]*?)*\}", re.DOTALL,
    )
    ...
    for filename in os.listdir(bnd_dir):
        if filename.endswith(f"{drug_interest}.bnd"):
            ...
            mutations_kras_ids, mutations_egfr_ids = identif_mutations_kras_egfr(
                mutations_data, patients_ids
            )
