...
def personalized_patients_genes_cfgs(montagud_data, original_data_dir, results_dir, patients_ids, rna_seq_data_filtered, drug_name):
  montagud_data = montagud_data.rename(columns={'Target node': 'Target_node', 'Interaction type': 'Interaction_type'})
  ...
  rna_seq_data_filtered = rna_seq_data_filtered[rna_seq_data_filtered.index.isin(patients_ids)]

  rna_seq_data_max = rna_seq_data
  rna_seq_data_max['gene_max'] = rna_seq_data_max.groupby('gene_symbol')['rsem_tpm'].transform('max')
