...
def create_boxplot_three_groups(
    ...
    if (
        ...
        print(
            " Error: 'res_data', 'sens_data', or 'healthy_data' is not a pandas DataFrame."
        )
        print(f"Type of res_data: {type(res_data)}")
        print(f"Type of sens_data: {type(sens_data)}")
        print(f"Type of healthy_data: {type(healthy_data)}")
        return

    if data_kruskal_significant.empty:
        print(" No significant phenotypes found for plotting.")
        return
    ...
    for i, phenotype in enumerate(phenotypes):
        ...
        if current_phenotype_significant_data.empty:
            print(f" No significant data for phenotype: {phenotype}. Skipping.")
            continue
        ...
        for condition in significant_conditions:
            try:
                ...
                if condition in res_data.index and phenotype in res_data.columns:
                    ...
                    print(
                        f" Missing resistant data for condition '{condition}', phenotype '{phenotype}'. Skipping for this condition."
                    )
                    continue
                ...
                print(f"Problematic res_value: '{res_data.loc[condition, phenotype]}'")
